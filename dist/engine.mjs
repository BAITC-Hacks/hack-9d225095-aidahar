...
function optionalBool(v) {
  // Export tools use True/False, 1/0, localized values, blanks or null.
  // Optional metadata must never block importing a contractor.
  return v===true || v===1 || (typeof v==='string' && ['true','1','yes','да','истина'].includes(norm(v)));
}
export function validateProfiles(raw) {
  ...
  return raw.map((p,i)=>{
    try {
      ...
      // These are project provenance flags, not required fields in organizers' exports.
      for(const f of ['synthetic','city_imputed','price_imputed']) q[f]=optionalBool(p[f]);
